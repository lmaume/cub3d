...
/*   Updated: 2025/01/27 18:24:13 by mlapique         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		float grad)
{
	...
	while (i < line->p2.x - 1)
	{
		...
		if ((i < WIDTH - 1 && floor(place) < HEIGHT - 1) && ((i > 0
					&& floor(place) > 0)))
		{
		...
		mlx_image_t *image, float grad)
{
	...
	{
		usefull = 1 - (place - floor(place));
		if ((i < HEIGHT - 1 && floor(place) < WIDTH - 1) && ((i > 0
					&& floor(place) > 0)))
		{
...
void	first_point_steep(t_line_necessary *line, int xend, mlx_image_t *image,
		int truc)
{
	if ((truc + 1 < HEIGHT - 1 && xend < WIDTH - 1) && ((truc > 0
				&& xend > 0)))
	{
